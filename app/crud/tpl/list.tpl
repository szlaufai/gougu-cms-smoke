...
<script>
	...
	function gouguInit() {
		...
		table.on('tool(<model>)', function(obj) {
			var data = obj.data;
			if (obj.event === 'read') {
				tool.side('/admin/<model>/read?<pk>='+obj.data.<pk>);
			}
			else if (obj.event === 'edit') {
				tool.side('/admin/<model>/edit?<pk>='+obj.data.<pk>);
			}
				...
				layer.confirm('确定要删除该记录吗?', {
					...
					tool.delete("/admin/<model>/del", { <pk>: data.<pk> }, callback);
					layer.close(index);
				});
			}
			return false;
